...
export default {
  ...
  fs_list: async ({ input }) => {
    ...
    try {
      ...
      const listEntries = async (dir, prefix = "") => {
        ...
        for (const item of items) {
          ...
          const name = prefix ? path.join(prefix, item) : item;
          const entry = {
            name,
          ...
          if (recursive && stats.isDirectory()) {
            details.push(...await listEntries(itemPath, name));
          }
        }
      ...
      const details = await listEntries(safe);
